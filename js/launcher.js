...
window.addEventListener("simply-content-loaded", function() {
    api = {
        fetcher : solidAuthFetcher,
        session : false,
        webId : localStorage['solid-applauncher.webId'],
	url : localStorage['solid-applauncher.storageUrl'],
        issuer : localStorage['solid-applauncher.issuer'],
        haveSession : function(fetcher) {
        ...
        logout : function() {
            return solidAuthFetcher.logout()
        },
        get : function(path) {
            return api.login()
            .then(function(fetcher) {
                return api.fetcher.fetch(api.url + path);
            });
        },
    ...
    launcher = simply.app({
        routes: {
            ...
            '/#launch/' : function(params) {
                ...
                editor.pageData.pageTitle = "Solid App Launcher";
            },
            '/' : function(params) {
                api.haveSession()
                .then(function(session) {
                    if (session) {
                        document.location.hash = "launch";
                    } else {
        ...
        commands: {
            allowAndLaunch : function(el) {
                ...
                window.open(app.launchUrl + "?webid=" + escape(api.webId));
            },
            launch : function(el) {
                var launchUrl = el.getAttribute("data-solid-url");
                var app = editor.pageData.apps.filter(function(app) {
            ...
            logout : function(el) {
                api.logout();
		simply.route.goto(document.location.pathname);
            },
            connect : function(el) {
                api.webId = editor.pageData.webId;
                api.url = editor.pageData.storageUrl;
                api.issuer = editor.pageData.issuer;

                localStorage['solid-applauncher.webId'] = api.webId;
                localStorage['solid-applauncher.storageUrl'] = api.url;
                localStorage['solid-appluncher.issuer'] = api.issuer;
